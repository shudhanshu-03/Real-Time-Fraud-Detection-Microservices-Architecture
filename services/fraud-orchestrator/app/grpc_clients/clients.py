...
class FraudGrpcClients:
    ...
    async def connect(self):
        ...
        self.rule_engine_channel = grpc.aio.insecure_channel(f"rule-engine:50051")
        self.ml_scoring_channel = grpc.aio.insecure_channel(f"ml-scoring-service:50052")
        self.graph_analysis_channel = grpc.aio.insecure_channel(f"graph-analysis:50053")
    ...
    async def disconnect(self):
        if self.rule_engine_channel: await self.rule_engine_channel.close()
        if self.ml_scoring_channel: await self.ml_scoring_channel.close()
        if self.graph_analysis_channel: await self.graph_analysis_channel.close()
